                ...
                {userResults.map(p => {
                  ...
                  return (
                    <Link key={p.id} href={`/user/${p.username}`} onClick={() => { setUserSearch(''); setUserResults([]) }}
                      ...
                      {pMem ? <div className="avatar avatar-sm" style={{background:'var(--amber)',color:'#fff',border:'none',fontWeight:800,fontSize:9}}>{pMem}</div> : <div className="avatar avatar-sm">{pi}</div>}
                      <div style={{ flex: 1 }}>
              ...
              return (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--rule-light)' }}>
                  <Link href={`/user/${m.username}`} style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, textDecoration: 'none' }}>
                    {mMem ? <div className="avatar avatar-sm" style={{background:'var(--amber)',color:'#fff',border:'none',fontWeight:800,fontSize:9}}>{mMem}</div> : <div className="avatar avatar-sm">{mi}</div>}
                    <div>
          ...
          {teamMembers.length > 0 && (<>
            ...
            {teamMembers.map(m => {
              ...
              return (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--rule-light)' }}>
                  <Link href={`/user/${m.username}`} style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, textDecoration: 'none' }}>
                    {mMem2 ? <div className="avatar avatar-sm" style={{background:'var(--amber)',color:'#fff',border:'none',fontWeight:800,fontSize:9}}>{mMem2}</div> : <div className="avatar avatar-sm">{mi}</div>}
                    <div>
                      <span className="author-name" style={{ fontSize: 13 }}>{m.display_name || m.username}</span>
